...
| `api/main.py` → `/volume-footprint/{config,contracts,levels,oi-ladder,snapshot,tilt-history}` | 3ST has one FastAPI app; routes cannot live outside it |
...
| `analysis/volume_profile/tilt_history.py` | Session tilt curves + the comparison. `analysis/` convention, and it must import under uvicorn, pytest and the scheduler |
| `scripts/backfill_volume_tilt.py` | One-shot history seed. A script, not a route: it is not repeatable — the reachable window shrinks with every expiry |
| `data/volume_tilt_history.json` | Owned by `tilt_history.py`. Path resolved lazily so tests can redirect it |
| `execution/scheduler.py` | Ticks the tilt sampler so the curve fills in when nobody has the page open |
| `Pixel Perfect UI/src/components/volume/TiltHistoryCard.tsx` | Renders the comparison; imported by the route file |
| `tests/test_volume_tilt_history.py` | 14 tests — partial-vs-complete refusal, null/label discipline, sampler behaviour |
